...
public class CheckVariableTest {
    ...
    public void should_return_a_list_of_fields_for_each_property_in_a_variable() {
        ...
        assertField(list.get(0), list("field 1", "foo"), list("field 2", 1.0), list("class", foo.getClass()));
    }

    @Test
    public void should_return_a_list_of_fields_for_each_property_of_each_object_in_a_list() {
        List<List<List<Object>>> list = new CheckVariable(fooBar, "elements").query();

        assertEquals(2, list.size());
        assertField(list.get(0), list("field 1", "foo"), list("field 2", 1.0), list("class", foo.getClass()));
        assertField(list.get(1), list("field 1", "bar"), list("field 2", 2.0), list("class", bar.getClass()));
    }
